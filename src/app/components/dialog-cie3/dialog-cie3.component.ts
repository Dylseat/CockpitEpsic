...
@Component({
  selector: 'app-dialog-cie3',
  standalone: true,
  imports: [MatFormFieldModule, FormsModule, MatDialogModule, MatInputModule, MatSelectModule, CommonModule],
  templateUrl: './dialog-cie3.component.html',
  styleUrls: ['./dialog-cie3.component.scss'],
  providers: [ModuleService]
})
export class DialogCie3Component implements OnInit{
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data:any ,private dialogRef: MatDialogRef<DialogCie3Component>, private moduleService: ModuleService,) {}

  // Récupérez les modules CIE de la base de données
  ngOnInit(): void {
    // Appelez la méthode du service pour récupérer les modules
    this.moduleService.getModulesCie3annes().subscribe(modules => {
