...
@Component({
  selector: 'app-dialog-cie',
  standalone: true,
  imports: [MatFormFieldModule, FormsModule, MatDialogModule, MatInputModule, MatSelectModule, CommonModule],
  templateUrl: './dialog-cie.component.html',
  styleUrls: ['./dialog-cie.component.scss'],
  providers: [ModuleService]
})
export class DialogCieComponent implements OnInit{
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data:any ,private dialogRef: MatDialogRef<DialogCieComponent>, private moduleService: ModuleService,) {}

  // Récupérez les modules CIE de la base de données
  ngOnInit(): void {
    // Appelez la méthode du service pour récupérer les modules
    this.moduleService.getModulesCie1annes().subscribe(modules => {
